...
static int npcm7xx_jtag_switch_pin_func(struct jtag_info *jtag, u8 mode)
{
	...
	if (mode == MODE_PSPI) {
		...
		/* TCK */
		gc = gpiod_to_chip(jtag->pins[pin_TCK]);
		offset = desc_to_gpio(jtag->pins[pin_TCK]) - gc->base;
		pinctrl_gpio_free(gc, offset);

		/* TDI */
		gc = gpiod_to_chip(jtag->pins[pin_TDI]);
		offset = desc_to_gpio(jtag->pins[pin_TDI]) - gc->base;
		pinctrl_gpio_free(gc, offset);

		/* TDO */
		gc = gpiod_to_chip(jtag->pins[pin_TDO]);
		offset = desc_to_gpio(jtag->pins[pin_TDO]) - gc->base;
		pinctrl_gpio_free(gc, offset);

		ret = pinctrl_select_state(jtag->pinctrl, state);
		if (ret)
			return ret;
		...
		jtag->tms_level =
			gpiod_get_value(jtag->pins[pin_TMS]);
	}
