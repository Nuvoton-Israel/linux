				...
				size_t count, loff_t *ppos)
{
	...
	if (copy_to_user((void __user *)buf, (const void *)mbox->memory, count))
	{
				...
				size_t count, loff_t *ppos)
{
	...
	if (copy_from_user((void *)mbox->memory, (void __user *)buf, count))
	{
		spin_unlock_irqrestore(&mbox->lock, flags);
		return -EFAULT;
	}
...
static irqreturn_t npcm7xx_mbox_irq(int irq, void *arg)
{
	...
	regmap_update_bits(mbox->regmap, NPCM7XX_MBOX_BMBXSTAT,
